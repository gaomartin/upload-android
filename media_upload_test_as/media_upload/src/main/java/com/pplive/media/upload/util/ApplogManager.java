...
public class ApplogManager {
	...
	public void writeBaseInfo(BufferedWriter os) throws IOException {
		...
		if (os != null) {
			...
			os.newLine();
			os.write(getTimeTag() + TAG_BASE + "IP:"
			...
			String startEndTime = new Date(applogInfo.startEndTime)
					.toLocaleString();
			String costTime = ((applogInfo.startEndTime - applogInfo.startTime) / 1000)
					+ " seconds";
			os.write(getTimeTag()
					+ TAG_BASE
					+ String.format(
							"start time:%s, start end:%s, start cost:%s, start type:%s",
							startTime, startEndTime, costTime,
							applogInfo.startType));
			os.newLine();
			...
			String screenStr = "Screen inches:" + screenInches
					+ ",widthPixels:" + dm.widthPixels + ",heightPixels:"
	...
	public void init(Context context, String logPath) {
		try {
			...
			cacheLog = new File(logPath + "/media_uplod_sdk.log");
			cacheLogSaved = new File(logPath + "/media_uplod_sdk.tmp");
			if (!cacheLog.exists()) {
	...
	private void invokeLogcat(String value) {
		writeString(getTimeTag() + " " + LogUtils.getTag() + " " + TAG_LOGCAT
				+ value);
	}
	...
	private void invokeLogin() {
		// writeString(getTimeTag() + TAG_PASSPORT + "user login:" +
	...
	// + String.format("username:%s, islogin:%s, isVip:%s", username,
	// loginState, isVip));
	// os.newLine();
	...
	// {
	// String coolUser =
	// ParseUtil.parseInt(CacheDatabaseHelper.getUserLevel(context)) >= 1 ? "1"
	// : "0";
	// String area = get.areacode + "";
	// os.newLine();
	// os.write(getTimeTag() + TAG_PASSPORT +
	// String.format("coolUser:%s, areacode:%s", coolUser, area));
	// }
	// }
	// }
	...
	private class WriteFileThread extends Thread {
		...
		public void run() {
			while (true) {
				...
				try {
					if (cacheLog.length() > 2 * 1024 * 1024) {
						closeStream(fos);
