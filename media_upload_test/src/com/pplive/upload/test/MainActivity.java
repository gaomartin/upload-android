...
public class MainActivity extends Activity implements UpLoadManagerListener {
	...
	public void onStateChange(UploadInfo info) {
		LogUtils.error("onStateChange fid:" + info.getFid() + "  progress ="
				+ info.getProgress() + " state = " + info.getState());
	...
	protected void onDestroy() {
		...
		for (UploadInfo info : mDatas) {
			if (info.getState() == UploadInfo.STATE_UPLOADING
					|| info.getState() == UploadInfo.STATE_PAUSE) {
	...
	class GetReLunchDataTask extends
			AsyncTask<Object, Integer, List<UploadInfo>> {
	...
	class GetVideoPathTask extends AsyncTask<Object, Integer, List<String>> {
		...
		protected void onPostExecute(List<String> result) {
			super.onPostExecute(result);
			videoPaths = result;
			LogUtils.error("GetVideoPathTask videoPaths ="
					+ videoPaths.toString());
		}
	}
	...
	private void onPlayPauseClicked(UploadInfo info) {
		...
		// 暂停
		if (info.getState() == UploadInfo.STATE_UPLOADING
				|| info.getState() == UploadInfo.STATE_WAIT) {
			pauseUpload(info);
		} else if (info.getState() == UploadInfo.STATE_PAUSE
				|| info.getState() == UploadInfo.STATE_UPLOAD_FALI) {
			startUpload(info);
		}
	}
	...
	class UploadListAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			if (convertView == null) {
				...
				holder.imgPlayPause = (ImageView) convertView
			...
			case UploadInfo.STATE_UPLOADING:
				holder.imgPlayPause
						.setBackgroundResource(R.drawable.upload_play_pause_btn);
			...
			case UploadInfo.STATE_UPLOAD_SUCCESS:
				holder.imgPlayPause
						.setBackgroundResource(R.drawable.upload_complete_btn);
