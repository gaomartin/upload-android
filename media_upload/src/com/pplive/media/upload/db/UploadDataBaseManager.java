...
public class UploadDataBaseManager {
	...
	public void deleteUploadById(long id) {
		mDatabase.beginTransaction();
		try {
			mDatabase.delete(TABLE_UPLOAD, BaseColumns._ID + "=?",
	...
	public List<UploadInfo> searchAllUploads(String username) {
		...
		Cursor c = null;
		try {
			c = mDatabase.rawQuery("SELECT * FROM " + TABLE_UPLOAD + " WHERE "
					+ FIELD_USER_NAME + "=?", new String[] { username });
			if (c != null) {
				while (c.moveToNext()) {
					UploadInfo info = new UploadInfo();
					info.setFid(c.getString(c.getColumnIndex(FIELD_FID)));
					info.setId(c.getInt(c.getColumnIndex(BaseColumns._ID)));
					info.setLocalPath(c.getString(c
							.getColumnIndex(FIELD_LOCAL_PATH)));
					info.setName(c.getString(c.getColumnIndex(FIELD_NAME)));
					info.setPpfeature(c.getString(c
							.getColumnIndex(FIELD_PPFEATURE)));
					...
					info.setUserName(c.getString(c
							.getColumnIndex(FIELD_USER_NAME)));
					info.setCategoryId(c.getString(c
							.getColumnIndex(FIELD_CATEGORY_ID)));
					result.add(info);
				}
			}
			return result;
	...
	class UploadDataHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL(String.format(
					...
					TABLE_UPLOAD, BaseColumns._ID, FIELD_LOCAL_PATH,
					FIELD_NAME, FIELD_TOKEN, FIELD_APITK, FIELD_SIZE,
					FIELD_STATE, FIELD_PPFEATURE, FIELD_FID, FIELD_CATEGORY_ID,
					FIELD_PROGRESS, FIELD_USER_NAME));
		}
